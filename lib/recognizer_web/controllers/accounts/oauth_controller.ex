defmodule RecognizerWeb.Accounts.OAuthController do
  ...
  def callback(%{assigns: %{ueberauth_auth: auth}} = conn, _params) do
    ...
    with nil <- Accounts.get_user_by_service_guid(provider, uid),
         {:ok, user} <- register_oauth_user(user_params, provider, uid) do
  ...
  @doc """
  Github doesn't return `first_name` and `last_name` like other providers, they use `name` which is the full name.
